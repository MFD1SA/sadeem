...
const defaultFilters: ReviewFilters = {
  search: '', branchId: '', rating: null, sentiment: null, status: null,
};
...
export default function ReviewsCenter() {
  ...
  const filtered = useMemo(() => {
    return reviews.filter((r: DbReview) => {
      if (filters.search && !(r.review_text || '').includes(filters.search) && !r.reviewer_name.includes(filters.search)) return false;
  ...
  return (
    <div className="card h-[calc(100vh-120px)] flex overflow-hidden">
      ...
      <div className="w-80 min-w-[280px] border-l rtl:border-l ltr:border-l-0 ltr:border-r border-border overflow-y-auto flex-shrink-0">
        ...
        <ReviewsList reviews={filtered} branchMap={branchMap} selectedId={selectedReview?.id ?? null} onSelect={setSelectedReview} />
      </div>

      {/* Detail Column */}
      <div className="flex-1 overflow-y-auto">
        <ReviewDetail review={selectedReview} branchName={selectedReview ? branchMap[selectedReview.branch_id] || '' : ''} onUpdate={loadData} />
      </div>
    </div>
  );
}
