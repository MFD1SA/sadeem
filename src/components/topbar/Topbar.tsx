...
import {
  Search, Bell, Menu, Globe, ChevronDown, LogOut, User, Settings,
} from 'lucide-react';
...
export function Topbar({ onMenuClick }: TopbarProps) {
  ...
  // Sync search input when navigating away from reviews page
  useEffect(() => {
  ...
  const doSearch = (q: string) => {
    ...
    if (!trimmed) return;
    navigate('/dashboard/reviews', { state: { search: trimmed } });
  };
      ...
      <div className="mx-auto flex h-14 w-full max-w-[1400px] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold text-content-primary">{orgName}</div>
            <div className="truncate text-[11px] text-content-tertiary">{orgIndustry}</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          {/* Search — navigates to reviews page */}
          <div className="hidden md:flex items-center gap-2 rounded-xl border border-border bg-surface-secondary/70 px-3 py-2 w-64 lg:w-72 focus-within:border-brand-300 focus-within:ring-1 focus-within:ring-brand-200 transition-all">
            <Search
              size={15}
            ...
            <input
              ...
              placeholder={lang === 'ar' ? 'بحث في التقييمات…' : 'Search reviews…'}
              className="w-full border-none bg-transparent text-xs text-content-primary outline-none placeholder:text-content-tertiary"
            />
          </div>
          ...
          <div className="relative" ref={menuRef}>
            ...
            >
              ...
              <div className="hidden min-w-0 sm:block text-start">
                <div className="max-w-[120px] truncate text-xs font-medium text-content-primary">{userName}</div>
              </div>
              <ChevronDown size={14} className={`text-content-tertiary transition-transform ${showUserMenu ? 'rotate-180' : ''}`} />
            </button>
            ...
            {showUserMenu && (
              <div className="absolute top-full mt-2 ltr:right-0 rtl:left-0 z-50 w-60 overflow-hidden rounded-2xl border border-border bg-white shadow-xl ring-1 ring-black/5 animate-in fade-in slide-in-from-top-1 duration-150">
                <div className="border-b border-border bg-surface-secondary/40 px-4 py-3">
                  <div className="truncate text-sm font-semibold text-content-primary">{userName}</div>
                  <div className="truncate text-[11px] text-content-tertiary mt-0.5">{profile?.email}</div>
                </div>
                <div className="p-1.5">
                  <button
                    type="button"
